...
const CustomerForm: React.FC<CustomerFormProps> = ({
  ...
  // NomadRem branding desc
  const brandingDescription = useMemo(() => {
    if (tokenValidation?.generated_by_user?.name) {
      return (
        <div>
          This link to join NomadRem as a customer was generated by the agent{" "}
          <b>{tokenValidation?.generated_by_user?.name}</b>
        </div>
      );
    }
    return "This link to join NomadRem as a customer";
  }, [tokenValidation]);
  ...
  if (isValidating) {
    return (
      <div className="space-y-6">
        <BrandingSection description={brandingDescription} />
