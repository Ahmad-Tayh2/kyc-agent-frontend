...
const CustomerRecipientStep = (props: CustomerRecipientStepProps) => {
  ...
  const setSendCountry = useSendRemittanceStore((state) => state.setSendCountry);
  const setReceiveCountry = useSendRemittanceStore((state) => state.setReceiveCountry);
  const setRemittanceMethod = useSendRemittanceStore((state) => state.setRemittanceMethod);
  const setPayoutAgent = useSendRemittanceStore((state) => state.setPayoutAgent);
  ...
  const { data: recipientsResponse, isLoading: recipientsLoading } = useGetCustomerRecipients(
  ...
  } = useInfiniteRecipients(activeSearchTerm, hasSearched && isExpandedText && !!stepOne.customer);
  ...
  }, [customersData, customerId, stepOne?.recipient?.id, recipientId, stepOne?.customer?.id]);
  ...
  const paymentMethodOptions = useMemo(() => {
    ...
    if (recipientMethodsData.payout_agents && recipientMethodsData.payout_agents.length > 0) {
  ...
  useEffect(() => {
    if (stepOne?.customer?.id) {
      const customer = customersData?.data?.find(
        (c: CustomerType) => c.id.toString() === stepOne?.customer?.id.toString(),
      );
      if (customer) {
    ...
    if (recipient) {
      ...
      if (recipient.address.country?.name) {
        const recipientCountryInAllowed = receiveCountriesData?.find(
          (item: RemittanceCountry) => item.name === recipient.address.country.name,
        );
  ...
  const handleSendCountrySelect = (countryId: string | number) => {
    const countryItem = sendCountriesData?.find((item: RemittanceCountry) => item.id === countryId);
    if (countryItem) {
      setSendCountry({
        id: typeof countryItem.id === "string" ? parseInt(countryItem.id) : countryItem.id,
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          <label className="block text-sm font-medium text-gray-700">Customer/Sender</label>
          <SearchableSelect
            ...
            <div className="mt-2">
              ...
              >
                <span className="text-sm text-gray-600 underline cursor-pointer">
                  Search recipient for another customer and add it to the selected customer
                </span>
                {isExpandedText ? (
                ...
                <div className="mt-3 pt-3 border-t border-gray-200">
                  ...
                  {searchedRecipients.length > 0 ? (
                    <div className="space-y-2 max-h-96 overflow-y-auto">
                      ...
                      {searchedRecipients.map((recipient: RecipientDataType) => (
                        <div
                          key={recipient.id}
                        ...
                        >
                          <div>
                            ...
                            <span className="text-gray-500 ml-2 font-sm">
                              - (+{recipient.country_phone_code}) {recipient.phone_number}
                            </span>
                          </div>
                          {recipientOptions.some((r) => r.value === recipient.id) ? (
                            ...
                            <Button
                              ...
                              onClick={() => handleAttachRecipient(recipient.id)}
                              disabled={attachRecipientMutation.isPending}
                            >
                              {attachRecipientMutation.isPending ? "Adding..." : "Add"}
                            </Button>
                          )}
                        </div>
                      ))}
                  ...
                  ) : hasSearched && searchedRecipients.length === 0 && !isSearching ? (
                    <p className="text-sm text-gray-500 text-center py-3">
                      No recipients found. Try a different search term.
                    </p>
                  ) : !hasSearched ? (
                    <p className="text-sm text-gray-500 text-center py-3">
                      Enter a search term and click "Search" to find recipients from other
                      customers.
                    </p>
                  ) : null}
                </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Sending Country */}
        <div className="space-y-2">
          <label className="block text-sm font-medium text-gray-700">Sending Country</label>
          <SearchableSelect
        ...
        {/* Receiving Country */}
        <div className="space-y-2">
          <label className="block text-sm font-medium text-gray-700">Receiving Country</label>
          <SearchableSelect
      ...
      <div className="space-y-2">
        ...
        <SearchableSelect
          ...
          value={
            stepOne.selectedPaymentMethodType === "remittance_method" && stepOne.remittanceMethod
              ? `rm_${stepOne.remittanceMethod.id}`
              : stepOne.selectedPaymentMethodType === "payout_agent" && stepOne.payoutAgent
      ...
      {isStepValid("customer") && (
        <div className="mt-4 p-3 bg-green-50 border border-green-200 text-sm text-green-800 font-medium flex items-center gap-1 rounded-md">
          <CheckedIcon /> <span> Step completed! You can now proceed to the next step.</span>
        </div>
      )}
