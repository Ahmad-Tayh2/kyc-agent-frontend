...
export const useSendRemittanceNavigation = () => {
  ...
  const completedSteps = useSendRemittanceStore((state) => state.completedSteps);
  ...
  const isStepCompleted = useSendRemittanceStore((state) => state.isStepCompleted);
  ...
  const goToNextStep = useCallback(() => {
    ...
    return true;
  }, [currentStep, isStepValid, isStepCompleted, markStepCompleted, setCurrentStep]);
